...
function createContext() {
  const context = vm.createContext({
    console, Date, appDataSchemaVersionKey: "appDataSchemaVersion", currentAppDataSchemaVersion: "8.2", APP_VERSION: "8.2.0",
    ...
    reviewQueueKey: "reviewQueue", professionalResultsKey: "studyProfessionalResults", englishWordRecordsKey: "studyEnglishWordRecords", englishReadingRecordsKey: "studyEnglishReadingRecords", politicsRecordsKey: "studyPoliticsRecords", outputRecordsKey: "studyOutputRecords", ankiCandidatesKey: "studyAnkiCandidates",
...
test("real P0 backup migrates to P1 with empty result stores and all trusted invariants", () => {
  ...
  assert.equal(first.values.appDataSchemaVersion, "8.2");
  ...
  assert.deepEqual(JSON.parse(first.values.studyAnkiCandidates), []);
