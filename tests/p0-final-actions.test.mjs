...
const focusTimerSource = fs.readFileSync(new URL("../js/focus-timer-core.js", import.meta.url), "utf8");
...
test("the next-task link reveals its exact timetable row without changing execution state", () => {
  assert.match(tasksSource, /row\.dataset\.taskId = String\(task\.id \|\| task\.taskId \|\| ""\)/);
  assert.match(tasksSource, /row\.tabIndex = -1/);
  assert.match(tasksSource, /function revealTodayPlanTask\(taskId\)[\s\S]*panel\.open = true[\s\S]*item\.dataset\.taskId === normalizedId[\s\S]*row\.focus\(\{ preventScroll: true \}\)[\s\S]*row\.scrollIntoView\(\{ behavior: "smooth", block: "center" \}\)/);
  assert.match(source, /link\.addEventListener\("click", \(event\) => \{[\s\S]*revealTodayPlanTask\(priority\.targetId\)[\s\S]*event\.preventDefault\(\)/);
  assert.match(styleSource, /\.task-row\.is-revealed[\s\S]*@keyframes reveal-task-row[\s\S]*prefers-reduced-motion/);
  const revealBlock = tasksSource.slice(tasksSource.indexOf("function revealTodayPlanTask"), tasksSource.indexOf("function renderTasks"));
  assert.doesNotMatch(revealBlock, /saveTodayPlan|writeJson|localStorage|setTaskStatus|startPomodoro|startImmersiveFocus/);
});
...
test("rolling review requires closed-book evidence before a result can be saved", () => {
  ...
  assert.match(p0ResultsSource, /直接自由专注/);
  assert.match(p0ResultsSource, /start\.dataset\.reviewFocusMode = "five-minute"/);
  assert.match(p0ResultsSource, /freeFocus\.dataset\.reviewFocusMode = "free"/);
  assert.match(p0ResultsSource, /const directFree = start\.dataset\.reviewFocusMode === "free"[\s\S]*startReviewFreeFocusRound[\s\S]*startReviewFiveMinuteRound[\s\S]*const started = startReview\(review\)/);
  assert.match(tasksSource, /function startReviewFiveMinuteRound/);
  assert.match(tasksSource, /function startReviewFreeFocusRound\(review\) \{\s*return startReviewFocusRound\(review, \{ directFree: true \}\);\s*\}/);
  assert.match(tasksSource, /function startReviewFocusRound\(review, options = \{\}\)[\s\S]*state\.active\.reviewId !== reviewId[\s\S]*pendingFocusReview = \{[\s\S]*reviewId,[\s\S]*options\.directFree === true\) setFocusTimingMode\(FREE_FOCUS_MODE\)[\s\S]*startPomodoro\(\)/);
  assert.match(tasksSource, /已有未结束的专注轮，请先继续或结束当前专注/);
  assert.match(styleSource, /\.review-evidence-box/);
  assert.match(styleSource, /\.review-start-actions \{[^}]*justify-self: end/);
  const startBlock = tasksSource.slice(tasksSource.indexOf("function startReviewFocusRound"), tasksSource.indexOf("function setFocusReviewCardVisibility"));
  assert.doesNotMatch(startBlock, /saveDueReviewResult|applyReviewResult|writeJson\(reviewQueueKey/);
...
test("review focus closes directly into evidence result and the next review handoff", () => {
  ...
  assert.match(tasksSource, /function startReviewFocusRound\(review, options = \{\}\)[\s\S]*state\.active\.reviewId !== reviewId[\s\S]*pendingFocusReview = \{[\s\S]*reviewId,[\s\S]*sessionId: ""/);
...
test("review focus identity and unresolved evidence survive a safe page recovery", () => {
  assert.match(focusTimerSource, /const FOCUS_TIMER_VERSION = 4/);
  assert.match(focusTimerSource, /FOCUS_TIMER_COMPATIBLE_VERSIONS = new Set\(\[3, FOCUS_TIMER_VERSION\]\)/);
  assert.match(focusTimerSource, /contextKind,[\s\S]*contextId: contextKind \? contextId : ""/);
  assert.match(focusTimerSource, /const hasContext = raw\.timerVersion === FOCUS_TIMER_VERSION/);
  assert.match(focusTimerSource, /contextKind: next\.contextKind,[\s\S]*contextId: next\.contextId/);
  assert.match(focusTimerSource, /sameContext[\s\S]*&& sameContext/);
  assert.match(tasksSource, /function startPomodoro\(\)[\s\S]*contextKind: pendingFocusReview \? "due-review" : ""[\s\S]*contextId: pendingFocusReview \? pendingFocusReview\.reviewId : ""/);
  assert.match(tasksSource, /function saveFinalizedFocusSession\(segment\)[\s\S]*contextKind: segment\.contextKind === "due-review"[\s\S]*contextId: segment\.contextKind === "due-review"/);
  assert.match(tasksSource, /function restorePomodoroStateFromStorage\(\)[\s\S]*restorePendingFocusReviewFromTimerState\(focusTimerState\)[\s\S]*isPendingFocusReviewCurrent\(\)[\s\S]*settleStaleRestoredFocusReview\(\)[\s\S]*restorePendingFocusReviewResultCard\(\)/);
  assert.match(tasksSource, /function getUnresolvedFocusReviewSessions\(reviewId\)[\s\S]*session\.contextKind === "due-review"[\s\S]*session\.wrapupSaved !== true/);
  assert.match(tasksSource, /function getFocusReviewSessionSummary\(reviewId, fallbackSession = null\)[\s\S]*sessions\.reduce\(\(sum, item\) => sum \+ Math\.max\(0, Number\(item\.seconds\) \|\| 0\), 0\)/);
  assert.match(tasksSource, /function updateFocusSessionsWrapup\(sessionIds, completed, nextStep\)[\s\S]*ids\.has\(String\(session\.id \|\| ""\)\)[\s\S]*wrapupSaved: true/);
  assert.match(tasksSource, /function showFocusRecoveryIfNeeded\(\)[\s\S]*reviewState\.active\.reviewId === pendingFocusReview\.reviewId[\s\S]*`复盘 · \$\{reviewState\.active\.reviewLevel\}/);
  assert.match(tasksSource, /function showFocusReviewResultCard\(session\)[\s\S]*getFocusReviewSessionSummary\(pendingFocusReview\.reviewId, session\)[\s\S]*focusReviewResultDuration/);
